import { Controller, Post, Get, Body, Param, UseGuards, Request, UseInterceptors, UploadedFiles, ParseIntPipe } from '@nestjs/common';
...
export class CorridaVistoriaController {
    ...
    FilesInterceptor('files', 10, {
      ...
      fileFilter: (req, file, callback) => {
        if (!file.originalname.match(/\.(jpg|jpeg|png)$/)) {
          return callback(new Error('Apenas imagens JPG, JPEG e PNG são permitidas!'), false);
        }
        callback(null, true);
      },
    }),
  ...
  async enviarFotos(
    ...
    return await this.corridaVistoriaService.salvarFotos(idCorridaVistoria, files);
  }
